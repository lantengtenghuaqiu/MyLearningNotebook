...
public:
    ...
    int InitGlfw(const int &width, const int &height, const char *title, GLFWmonitor *glfwmonitor = nullptr, GLFWwindow *share = nullptr)
    {
        if (glfwInit() != FAIL)
        {
            ...
            if (this->window != NULL)
            {
                printf("Created the window and make context : %p\n",&this->window);
                glfwMakeContextCurrent(this->window);
            }
            else
            {
                printf("window is NULL!!! : %d\n", __LINE__);
                glfwTerminate();
            }
        }
        else
        {
            printf("glfwInit with something wrong,initiation is failed!!! : %d\n", __LINE__);
            glfwTerminate();
        }
...
public:
    ...
    void BindVBO(unsigned int &VBO, const Binder binder, const T *data)
    {
        if (binder.AttributeChecker())
        {
            printf("Bind VBO : %d\n",VBO);
            glBindBuffer(GL_ARRAY_BUFFER, VBO);
            glBufferData(GL_ARRAY_BUFFER, sizeof(data), data, binder.DRAWYPE);

            glVertexAttribPointer(binder.Location, binder.DataSize, binder.DATATYPE, binder.NORMILAZED, sizeof(float) * 3, (void *)0);
            // 第二个参数表示分量vec2传2, vec3传3, vec4传4
            glEnableVertexAttribArray(binder.Location);
            glBindBuffer(GL_ARRAY_BUFFER, 0);
        }
    }
    void BindEBO(unsigned int &EBO, const unsigned int *index, int ebo_draw_type = GL_STATIC_DRAW)
    {
        printf("Bind EBO : %d\n" , EBO);
    ...
    void CompileAndAttachShader(unsigned int shader_program, unsigned int shader_id, char *shader_data)
    {
...
int main()
{
    ...
    if (glfw.InitGlfw(width, height, "OpenGLClass"))
    {
        ...
        glad.BindEBO(attri.EBO[0], indices, GL_STATIC_DRAW);
        glad.BindVBO(attri.VBO[0], VBO0, vertices);
        glad.UnbindVAO();

        Shader shaders;
        // Vertex Shader
        file.GetContent("G:\\user\\desktop\\C++\\GraphicLearning\\D_OpenGL\\3.Camera\\Windows\\shaders.vertex", "rb", shaders.ShaderData);
        glad.CompileAndAttachShader(shaders.ShaderProgram, shaders.VertexShader, shaders.ShaderData);

        // Fragment Shader
        file.GetContent("G:\\user\\desktop\\C++\\GraphicLearning\\D_OpenGL\\3.Camera\\Windows\\shaders.fragment", "rb", shaders.ShaderData);
        glad.CompileAndAttachShader(shaders.ShaderProgram, shaders.FragmentShader, shaders.ShaderData);
        ...
        printf("Check the window address : %p\n",&glfw.window);
