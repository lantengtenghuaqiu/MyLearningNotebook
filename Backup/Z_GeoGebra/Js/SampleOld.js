...
function Destory(state) {
    if (ggbApplet.exists(`${state[2]}`) && ggbApplet.getValue(`${state[2]}`)) {
        ...
        for (i = 0; i < window.Global.OBJECTS.length; i++) {
            if (ggbApplet.exists(`${window.Global.OBJECTS[i]}`))
                ggbApplet.deleteObject(`${window.Global.OBJECTS[i]}`);
        }
    }
            ...
            if (type == "create") {
                switch (number) {
                    case 2:
                        ...
                        window.Global.OBJECTS.push(`${name}`);
                        break;
                    case 3:
...
function CreateText(contents) {
    if (Array.isArray(window.Global.text_index)) {
        if (!ggbApplet.exists(`t${window.Global.text_index[0]}`)) {
            ggbApplet.evalCommand(`t${window.Global.text_index[0]}="${contents}"`);
            ggbApplet.setVisible(`t${window.Global.text_index[0]}`, false);
            window.Global.text_index[0] = window.Global.text_index[0] + 1;
            window.Global.OBJECTS.push(`t${window.Global.text_index}`);
        }
    } else {
