...
long long __stdcall WndProc(HWND__ * hwnd , unsigned int msg , unsigned long long wParam , long long lParam)
{
    switch (msg)
    {
        case WM_PAINT:
        {
            ...
            HDC__ * hdc_buffer = CreateCompatibleDC(hdc);
            HBITMAP__ * bmp_buffer = CreateCompatibleBitmap(hdc,window_width,window_height);
            HGDIOBJ__ * old_buffer = SelectObject(hdc_buffer,bmp_buffer)
