<<C/C++笔记>>:
	->#目录索引#:
		...
		->#终端编译专栏#:
			-#通过终端编译c++文件,生成.exe文件:
				...
				8.编译时设置全局宏:
					...
					MinGW编译改为宽字符:-DUNICODE -D_UNICODE;
			->#库文件生成:
				MinGW:
					1.MinGW下编译生成静态库:
		...
		->#指针专栏#:
			...
			->nullptr和NULL:
				首先nullptr是空指针,本质是指向0x00000000的虚拟内存地址,这个地址不映射到物理内存地址中.

			->#指针变量和指针:
				1.对于创建一个指针它包含了两部分:
					int * p = new int;
					*p是指针对象,它本身存在栈区,通过&p可以获得这个指针对象,他在栈区的地址.
					使用new int创建一个能存放int大小的内存空间的地址,因为没有赋值所以会给一个默认值.当使用*p = 10;的时候,修改该指针指向的地址的内容.

					这时:
						&p对应的是该指针对象的地址;
						p是开辟的内存空间的地址
						*p是指开辟内存空间的地址,对应的内容;
				2.当指针指向某一个具体对象的地址的时候:
					int a = 10;
					int * p = &a;
					这时:
						这里的&p对应的是该指针对象的地址
				...
				3.释放内存和指针对象:
					对于int * p = new int;
					它是指针对象;内存地址;内存地址对应的内容;
					这三部分的集合.
					因此对于释放:
						free(p);
						释放内存中对应的地址和内容,这个时候p会变成空指针;
						delete(p);或free(p);
						接触虚拟内存和物理内存的映射;
						p=nullptr;
						将p的指针指向0x00000000的虚拟内存地址中,释放物理内存对应的映射;
			->#空指针:
				int * p = nullptr;
				这就是一个空指针,指向0x00000000的虚拟内存地址,这个地址不映射到物理内存地址中.
			
			->#delete和free的区别:
				两者都是解除指针和其对应的物理映射关系
				free()属于c,delete()属于c++.
				delete()在free的基础上,添加了对象析构函数启用的功能.(因为c没有类对象)
		...
		->#类专栏:
			->#拷贝构造函数:
				对于每一个class,在编译器构造的时候都会隐式的初始化一个默认构造函数:
					class CopyConstruction{
						"CopyConstruction(const CopyConstruction&)":拷贝构造原型
					}
				他和有参构造是互为独立的存在
				在进行调用构造函数初始化的时候并不会调用拷贝构造,
				而只有在使用已有的对象进行构造的时候才会调用:
					CopyConstruction Cc1;
					CopyConstruction Cc2(Cc1);这样的时候会调用拷贝构造
						在初始化Cc2的时候,不会隐式初始化任何新的 Cc1 对象,
						而是把已经存在的 Cc1 对象的引用直接传给 Cc2 的拷贝构造函数，
						拷贝构造函数通过这个引用，直接访问 Cc1 的成员变量，完成对 Cc2 的初始化。
				
				->#什么时候创建临时对象?
					当使用CopyConstruction Cc2()=CopyConstruction();的时候,会调用一个CopyConstruction的构造函数,如果是有参则调用有参,无参则调用无参.
					这个临时的对象的生命周期在;号结束时就结束,因此如果该类中包含析构函数,释放了成员指针,则Cc2中对应的成员指针就会变成野指针.
				->#禁用拷贝构造函数:
		...
		->#字符:
			->#什么是单字符和多字符
				->#单字符表示,一个字符由一个字节组成的.
					直接对应ASCII码,比如'a' 'b' 'c' '1'等
				->#多字符表示,一个字符由多个字节组成的.


			->#什么是窄字符和宽字符:
					窄字符和宽字符都是字节容器,本身不存储所谓的'a' 'b'这种字符,他们存储的是字符对应的字符码.
					作为容器他们本身就是对于字符对应的数字的存储:
						窄字符对应char,每个char都只能存储存储单字节对应ASCII码.
						宽字符对应wchar,为处理多字节字符比如中文,韩文等.
					->#窄字符存储
						但是窄字符可以存储宽字符,宽字符也可以存储窄字符:
							char word[]='中';
							因为'中'是一个多字符,又三个字节组成,因此char[0]和[1]以及[2]会依次'中'存储对应的三部分单字节.
							对于存储了char word[] = "中文";两个字符时候.
								因为中文字符本身就是1110开头的,每次读取到1110就代表是一个新字符.

					->#宽字符存储:
						宽字符不仅可以存储多字符也可以存储单字符.
							wchar word=L'中';
								一个word就可以存储一个多字节字符.不需要数组存储.
						注意需要添加L.


			->#字符编码:
				什么是字符编码:
					字符本身就是将人能看懂的语言变成了机器能看懂的0和1,因此字符需要有一个对应所谓0和1的东西.
					这个东西就是字符编码.
				具体的字符编码:
					->#Unicode;
